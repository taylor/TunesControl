   ...
   private void start(Collection serviceInfos) {
      ...
      for (Iterator iterator = serviceInfos.iterator(); iterator.hasNext();) {
         try {
            ...
            Log.d(TAG, "start() Registration exception: " + exception.getMessage());
         }
      }
      // Log.d(TAG, "start5");
   }
   ...
   private void closeMulticastSocket() {
      ...
      if (socket != null) {
         ...
         try {
            ...
            Log.d(TAG, "closeMulticastSocket() Close socket exception: " + exception.getMessage());
         }
         socket = null;
      }
   }
         ...
         for (DNSCache.CacheNode j = cache.find(info.getQualifiedName().toLowerCase()); j != null; j = j.next()) {
            ...
            if ((a.type == DNSConstants.TYPE_SRV) && !a.isExpired(now)) {
               ...
               if (s.port != info.port || !s.server.equals(localHost.getName())) {
                  Log.d(TAG, "makeServiceNameUnique() JmDNS.makeServiceNameUnique srv collision:" + a + " s.server="
                              + s.server + " " + localHost.getName() + " equals:"
   ...
   public void send(DNSOutgoing out) throws IOException {
      ...
      if (!out.isEmpty()) {
         ...
         try {
            ...
            Log.w(TAG, "send(DNSOutgoing) - JmDNS can not parse what it sends!!!", e);
         }
         socket.send(packet);
      }
   }
