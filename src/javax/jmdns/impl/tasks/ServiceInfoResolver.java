      ...
      try {
         if (this.jmDNSImpl.getState() == DNSState.ANNOUNCED) {
            if (count++ < 3 && !info.hasData()) {
               ...
               out.addAnswer((DNSRecord) this.jmDNSImpl.getCache().get(info.getQualifiedName(), DNSConstants.TYPE_SRV,
                        DNSConstants.CLASS_IN), now);
               out.addAnswer((DNSRecord) this.jmDNSImpl.getCache().get(info.getQualifiedName(), DNSConstants.TYPE_TXT,
                        DNSConstants.CLASS_IN), now);
               if (info.getServer() != null) {
                  out.addAnswer((DNSRecord) this.jmDNSImpl.getCache().get(info.getServer(), DNSConstants.TYPE_A,
                           DNSConstants.CLASS_IN), now);
               }
