...
public class Announcer extends TimerTask {
   public final static String TAG = Announcer.class.toString();
   ...
   public void run() {
      ...
      try {
         ...
         for (Iterator i = list.iterator(); i.hasNext();) {
            ...
            synchronized (info) {
               if (info.getState() == taskState && info.getTask() == this) {
                  info.advanceState();
                  Log.d(TAG, "run() JmDNS announcing " + info.getQualifiedName() + " state " + info.getState());
         ...
         if (out != null) {
            Log.d(TAG, "run() JmDNS announcing #" + taskState);
